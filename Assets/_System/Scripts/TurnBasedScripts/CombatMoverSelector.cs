...
public class CombatMoverSelector : MonoBehaviour
{
    ...
    protected void Update()
    {
        if (Input.GetMouseButtonDown(0))
        {
            ...
            {
                if (CombatManager.Instance.FriendlyUnitAt(GridManager.Instance.m_HoveredGridTile.m_GridPosition))
                {
                    SwapPiece(CombatManager.Instance.TrySelectUnitAtTile(GridManager.Instance.m_HoveredGridTile));
                    return;
                }
    ...
    public void EnterState(GridObject piece)
    {
        ...
        _moveLocations = CombatManager.Instance.MovesForUnit(_movingPiece);
    ...
    {
        enabled = false;
        CombatManager.Instance.DeselectUnit();
        _movingPiece = null;
